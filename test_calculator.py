...
#Exercise 4.2
@pytest.mark.parametrize('x, o',[[(0,5),0],[(mt.pi/2,5),1]])
def test_sin_tol(x,o):
    tol=1e-6
    assert abs(calculator.sin(x[0],x[1])-mt.sin(x[0]))<tol
    assert abs(calculator.sin(x[0],x[1])-o)<tol

@pytest.mark.parametrize('x',[[-1, 5],[-mt.pi/2, 5]])
def test_sin_neg(x):
